...
<div class="container">
    <div class="row">
        <div class="col-md-3">
    <select class="selectpicker fft_pref fft_select" id="category" title="Select categories..."
...
<input type="hidden" id="catheads" value="{{ $catheads }}" />
<input type="hidden" id="catvals" value="{{ $catvals }}" />
@endsection
...
$(document).ready(function() {
    feedbackTable = $('#feedback').DataTable({
        ...
        serverSide: false,
        searching: true,
        ajax: { url: '{{ route('fullinfo/getdata') }}',
               data: function ( d ) {
                        d.category = $('#category').val();
                        d.fft_yr = $('#fft_year').val();
                        d.fft_mth = $('#fft_month').val();
                        d.catheads = $('#catheads').val();
                        d.catvals = $('#catvals').val();
              },
        },
        columns: [
            {data:      'TRIGGER_DT_TM',
