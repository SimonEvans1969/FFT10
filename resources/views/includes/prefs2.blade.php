...
<span>
    ...
    <?php if (isset($prefs->{$category->CATEGORY_NAME}) && ($prefs->{$category->CATEGORY_NAME} == $category->CATEGORY_VALUE))
              echo ' selected ';
    ?>
...
<?php
    ...
    if (isset($chart) && $chart) { ?>
                    ...
                    chart_subtype : $('#chart_subtype').val(),
                    chart_sort : $('#chart_sort').val(),
                    chart_category : $('#chart_category').val(),
...
function setPrefs() {
    // disable button until this process completes
    $('#fft_submit').prop( "disabled", true );
         ...
         switch (opt_count) {
             case 1 :
                $(this).closest('span').hide();
                 break;
             /* REMOVED AS IF ONLY ONE OPTION, BUT DOES NOT APPLY TO ALL SCENARIOS THEN INCORRECTLY LIMITS VISIBLE OUTPUT
             ** EG. ONE WARD BUT CAN HAVE OP DATA FOR WHICH WARD WOULD BE BLANK
             case 2 :
    ...
    $('#fft_submit').prop( "disabled", false );
}
</script>