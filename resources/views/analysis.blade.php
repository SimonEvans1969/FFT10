...
<div class="container">
    <div class="row">
        <div class="col-md-3">
          <select class="selectpicker fft_pref fft_select" multiple title="Select categories..." id="category">
...
foreach ($categories as $category) {
    foreach ($selected as $sel_cat) {
        if ($sel_cat == $category->CATEGORY_NAME) {
                echo "<th cat-head='$category->CATEGORY_NAME' >$category->DESCRIPTION</th>";
                break;
        }
    }
}
...
$(document).ready(function() {
    feedbackTable = $('#feedback').DataTable({
        ...
        serverSide: false,
        searching: false,
        ajax: { url: '{{ route('analysis/getdata') }}',
               data: function ( d ) {
...
$(document).ready(function() {   
    ...
    $('#feedback tbody').on('dblclick', 'tr', function () {
        var cats = [];
        
        // Get the headings
        var catheads = [];
        var cnt = 0;
        $(this).closest('.dataTables_scroll').find('.dataTables_scrollHead').find('[cat-head]').each( function () {
            catheads[cnt++] = $(this).attr('cat-head');
            });
        ...
        $(this).children('td').each( function () {
           if (catheads[cnt]) {
               catvals[cnt++] = $(this).text();
           }
        });
        
        window.location.href = "{{url('fullinfo')}}" 
                        + '?catheads=' + encodeURIComponent(catheads)
                        + '&catvals=' + encodeURIComponent(catvals);
    } ) 
    <?php } ?>
